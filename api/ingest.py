...
def ingest_journal_into_duckdb(conn, last_seconds: int = 3600, limit: Optional[int] = None) -> Tuple[int, int]:
    ...
    for entry in _journalctl_json_lines(last_seconds=last_seconds, limit=limit, after_cursor=after_cursor):
        ...
        rows.append((ts, hostname, "journald", unit, facility, severity, pid, uid, gid, message, raw_json, fingerprint, cursor))
    ...
    if last_seen_cursor:
        conn.execute(
            "INSERT INTO ingest_state(source, cursor, updated_at) VALUES('journald', ?, CURRENT_TIMESTAMP) "
